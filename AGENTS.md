...
## First Principle — Minimum Sufficient Change

Default to the smallest change that closes the current user-visible or contract-visible need and passes the affected verification surface. Prefer reuse of existing seams, types, tests, and components over new layers.

Do not add abstractions, generalized frameworks, dependencies, evidence artifacts, compatibility machinery, configuration, or speculative future flexibility unless a current requirement or demonstrated risk requires them. Tests are sufficient evidence for ordinary deterministic development invariants; create parallel evidence/documentation state only when it has independent operational, governance, audit, or learning value.
