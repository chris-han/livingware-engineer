# Contributing to Livingware Engineer
...
A real integration must make Livingware skills discoverable and triggerable at the right moments. Prefer the harness native skill-discovery/matching mechanism when it exists. Only harnesses that lack equivalent native matching should inject the compact `using-superpowers` compatibility bootstrap at session start; do not add a redundant first-hop router on platforms that already select skills natively.
