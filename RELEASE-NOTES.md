...
- Integrate Design Thinking divergence/convergence with four evidence-maturity stages so early business-model exploration remains qualitative instead of producing false numerical accuracy.
- Add a zero-dependency deterministic simulation runner with restricted expressions, scenario overrides, non-finite-value rejection, and an explicit boundary against forecasting from uncalibrated models.
- Synchronize declared package/plugin version surfaces to 6.7.0.
...
- **New harness PRs require a session transcript** — most past new-harness integrations…5901 tokens truncated…ibilities before defining tasks. New "Scope Check" backstop: catch multi-subsystem specs that should have been decomposed during brainstorming
