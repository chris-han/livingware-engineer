...
4. Any failed or unverified requirement keeps the plan and host goal incomplete. Continue authorized work by default. Route failures to repair/debug/reverification; surface a blocker to the user only when it satisfies the User Intervention Necessity Test. A predeclared negative or inconclusive terminal disposition is closure, not a blocker, when its own predicate is proved. Do not narrow the stopping criterion to the implemented subset.
...
Evidence decides the claim; verification is not a reason to generate extra reports or stop for approval. Concise status may be reported without pausing; user interruption requires the User Intervention Necessity Test.
